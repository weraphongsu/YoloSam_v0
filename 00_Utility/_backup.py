...
BOX_LIST = [   "0_vietnam_areas_shp", "10_cambodia_areas_shp", "11_cambodia_areas_shp", "12_cambodia_areas_shp",
...
# Skip problematic boxes (only for Airbus)
SKIP_BOXES = []  # e.g., [19] if it's too large

# ================== PARAMETERS ==================
MAX_TILES_PER_DOWNLOAD = 1000
ZOOM_LEVEL = 18
...
def get_feature_collection(dataset_type, box_id):
    ...
    if dataset_type == "airbus":
        ...
        return ee.FeatureCollection(f'projects/earthengine-legacy/assets/projects/servir-mekong/khProject/data/{box_id}')
    else:
        raise ValueError(f"Unknown dataset type: {dataset_type}")

def get_output_name(dataset_type, box_id, source):
    """Generate output filename based on dataset type"""
    if dataset_type == "airbus":
        return f"airbus_box_{box_id}_{source.lower()}.tif"
    elif dataset_type == "myanmar":
        return f"{box_id}_{source.lower()}.tif"
    else:
        raise ValueError(f"Unknown dataset type: {dataset_type}")
...
for box_id in BOX_LIST:
    try:
        # Skip problematic boxes (only for Airbus)
        if DATASET_TYPE == "airbus" and box_id in SKIP_BOXES:
            print(f"\nSkipping Box {box_id} (too large - manually handle if needed)...")
            continue
        ...
        print(f"  Step 1: Getting bounding box coordinates...")
        try:
        ...
        print(f"  Area: {width:.6f}° x {height:.6f}° = {area_deg2:.8f} deg²")

        # Download from each selected source
        for source in DOWNLOAD_SOURCES:
            # Define output path with source suffix
            output_filename = get_output_name(DATASET_TYPE, box_id, source)
            image_path = os.path.join(base_output_dir, output_filename)
            ...
            try:
                if source == "Bing":
                    ...
                    success = download_bing_tiles(bbox, ZOOM_LEVEL, image_path, box_id)
                    if success:
                        # Standardize Bing (includes CRS conversion)
                        standardize_image(image_path, source)
                        print(f"  {source} image saved to: {output_filename}")
                    else:
                ...
                print(f"  Error downloading {source} for {box_id}: {str(e)}")
                import traceback
                traceback.print_exc()
                continue
...
if DATASET_TYPE == "airbus":
    airbus_files = [f for f in downloaded_files if f.startswith('airbus_')]
    print(f"  Airbus boxes: {len(airbus_files)} files")
elif DATASET_TYPE == "myanmar":
    myanmar_files = [f for f in downloaded_files if f.startswith('myanmar_')]
    print(f"  Myanmar fields: {len(myanmar_files)} files")
...
if SKIP_BOXES and DATASET_TYPE == "airbus":
